...
private:
    ...
    struct ValueLattice {
        ...
        ValueLattice() : Tag(UNDEFINED), ConstValue() {
        }

        explicit ValueLattice(lattice_t Tag): Tag(Tag), ConstValue() {
        }
    ...
    SetVector<BasicBlock *> BlockList; // work list of blocks
    SetVector<Instruction *> WorkList; // set of instructions that need reevaluation
    SetVector<BasicBlock *> Visited;

    ValueLattice &LatCell(Value *V) {
        auto [fst, snd] = ValueState.insert(std::make_pair(V, ValueLattice()));
        auto &LV = fst->second;
        return LV;
    }
    ...
    bool SCCP_Evaluate(Instruction *I) {
        ...
        if (auto *PN = dyn_cast<PHINode>(I)) {
            ...
            for (Value *Incoming: PN->incoming_values()) {
                Constant *C = ValueState.lookup(Incoming).ConstValue;
        ...
        for (Value *Op: I->operands()) {
            if (Constant *C = ValueState.lookup(Op).ConstValue) {
        ...
        if (auto *BO = dyn_cast<BinaryOperator>(I)) {
            Constant *C = ConstantFoldBinaryInstruction(
                BO->getOpcode(),
                ConstOperands[0],
                ConstOperands[1]
            );
    ...
    void SCCP_Instruction(Instruction *I) {
        if (isa<BinaryOperator>(I) || isa<PHINode>(I)) {
            if (SCCP_Evaluate(I)) {
                I->replaceAllUsesWith(ValueState[I].ConstValue);
                for (User *U: I->users()) {
...
llvmGetPassPluginInfo() {
    return {
        LLVM_PLUGIN_API_VERSION, "bajzc: Sparse Conditional Constant Propagation", "0.1", [](llvm::PassBuilder &PB) {
